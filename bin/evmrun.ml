...
let summarize_allocs ~depth () =
  ...
  Printf.printf "\n=== allocation cost centres (statmemprof, rate %.4g) ===\n" sampling_rate ;
  Printf.printf "sampled blocks: %d minor, %d major | samples: %d | est. total: %.1f MB\n\n"
    (List.length minor) (List.length major) total_samples
    (bytes_of_samples total_samples /. 1.0e6) ;
  ...
  Printf.printf "sampled major blocks: %d | samples: %d | est. total: %.1f MB\n\n"
    (List.length major) major_total (bytes_of_samples major_total /. 1.0e6) ;
  Printf.printf "  %14s %7s %9s %7s %8s %7s   %s\n" "est. bytes" "%" "#blocks" "min w" "avg w" "max w" "site" ;
  List.iteri
    (fun i (site, s, b, w, mx, mn) ->
      if i < 25 then begin
  ...
  Printf.printf "sampled promoted blocks: %d | samples: %d | est. total: %.1f MB\n\n"
    (List.length promoted) promoted_total (bytes_of_samples promoted_total /. 1.0e6) ;
  Printf.printf "  %14s %7s %9s %7s %8s %7s   %s\n" "est. bytes" "%" "#blocks" "min w" "avg w" "max w" "site" ;
  List.iteri
    (fun i (site, s, b, w, mx, mn) ->
      if i < 25 then begin
