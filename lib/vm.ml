...
struct
  module Memory : sig
    ...
    (** Check that the index (start + size - 1) does not overflow the range of an int or exceed the memory's
        current active bytes. Memory must be extended by a call to [extend_to] beforehand. *)
    let overflow_check mem start size_bytes =
      assert (start + size_bytes > start) ;
    ...
    let write_word_at (pos : U256.t) w (mem : t) : t =
      ...
      if is_aligned pos' then (
        overflow_check mem pos' 32 ;
        write_aligned pos' (U256.to_repr w) mem )
