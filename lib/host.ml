...
module Make (ChainParams : Chain.Monad.PARAMS) (Vm : Evmc.VM) = struct
  module rec Self : sig
    include SELF
    val call_from_eoa : Transaction.t -> Evmc.Message.t -> Evmc.Result.t TransactionState.M.t
  end = struct
    let self = (module Self : SELF)
    ...
    let set_storage addr key v =
      ...
      let$ () =
        account ~keep_empty:true addr |-- storage |-- B32.Map.at key
        := if B32.(v = zeros) then None else Some v
      in
    ...
    let try_precompile (address : Address.t) (msg : Evmc.Message.t) ~otherwise =
      ...
      | Some _ ->
          (* Delegated calls to precompiles are executed as if the corresponding contract was empty,
           as per EIP-7702. *)
          Vm.execute self msg Bytes.empty
      | None -> otherwise
      ...
      if U64.(pre_existent_account.nonce <> zero) || pre_existent_account.code <> Bytes.empty then
        ...
        let$ result =
          if transfer_ok then
            ...
            in
            Vm.execute self creation_msg msg.input_data
          else return Evmc.Result.(failure StatusCode.Insufficient_balance)
        in
    ...
    let call_impl ~(from_tx : Transaction.t option) (msg : Evmc.Message.t) =
      let$ () =
        ...
        when_
          ((msg.kind = Create || msg.kind = Create2) && Option.is_none from_tx)
          (increment_nonce msg.sender)
      in
