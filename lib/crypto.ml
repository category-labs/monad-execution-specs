...
(* We use the Zarith-backed U64.t for blake2 instead of the native Int64.t. This is not optimal, but it is
...
let rotr64 x (n : int) = U64.(logor (shift_right x n) (shift_left x Stdlib.(64 - n)))
...
let blake2f ~rounds ~(h : U64.t Iarray.t) ~(m : U64.t Iarray.t) ~t0 ~t1 ~final_block =
  let open U64 in
  let ( .$() ) = Iarray.get in
