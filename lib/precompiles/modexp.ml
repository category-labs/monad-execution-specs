  ...
  Precompile.(
    run msg
      (let$ base_length = parameter_length in
       ...
       let$ () =
         let gas_cost =
           ...
           let exponent_upper_256_bits =
             let exponent_start = (3 * 32) + base_length in
             Uint.of_bytes_be
               (Bytes.sub_with_zero_padding msg.input_data exponent_start (min exponent_length 32))
           in
       ...
       let result = if Uint.(modulus = zero) then Uint.zero else Uint.exp_mod base exponent ~modulo:modulus in
