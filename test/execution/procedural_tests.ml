...
module Wallet = struct
  ...
  let of_secret_key (pk : B32.t) =
    let secret_key = Key.read_sk_exn Crypto.context (Crypto.buffer_of_b32 pk) in
...
let send
 ...
 fun (state : state) ->
  ...
  Format.eprintf "Sending tx with nonce %s\n" (U64.to_string nonce) ;
  let tx_hash =
    let tx_encoded =
      Rlp.(
        encode
          (List
             ...
             ; Rlp.of_bytes data ] ) )
    in
    tx_encoded |> Crypto.keccak_256 |> Crypto.buffer_of_b32 |> Sign.msg_of_bytes_exn
  in
  let signature = Sign.sign_recoverable_exn Crypto.context ~sk:sender.secret_key tx_hash in
  let rs, recid = Sign.to_bytes_recid Crypto.context signature in
